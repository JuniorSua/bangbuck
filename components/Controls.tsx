...
export function Controls({
  ...
  return (
    <section className="card p-7 sm:p-10">
      ...
      <div className="grid gap-9 sm:grid-cols-2 sm:gap-14">
        <Slider
          label="Ship floor"
          ...
          help="Minimum task pass rate. Qualification uses the measured estimate; its confidence interval may cross this floor."
        />
        <Slider
          label="Craft floor"
          ...
          help="Minimum estimated WebDev preference against the board median. A model must also clear your Ship floor."
        />
      </div>

      <details className="mt-8 border-t pt-7 group" style={{ borderColor: "var(--border)" }}>
        <summary
        ...
        <div className="mt-8 grid gap-9 sm:grid-cols-2 sm:gap-14">
          <Slider
        ...
        <p className="mt-4 text-xs leading-relaxed" style={{ color: "var(--text-muted)" }}>
          <span className="font-mono">
            BangBuck = 100 × ship^{(1 - settings.craftWeight).toFixed(1)} × craft^
            {settings.craftWeight.toFixed(1)} ÷ (cost × (tokens / minTokens)^{settings.beta.toFixed(2)} × (steps / minSteps)^
            {settings.gamma.toFixed(2)})
          </span>
          , over configs clearing <em>both</em> floors. minTokens and minSteps are the smallest measured values across the full snapshot. The hard floors enforce both requirements; the geometric blend ranks capability within them.
        </p>
      </details>
    </section>
  );
...
export function TierSelector({ settings, onChange }: {
  settings: Settings;
  onChange: (s: Settings) => void;
}) {
      ...
      <div className="mb-7 flex flex-wrap items-center justify-between gap-5">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: "var(--text-secondary)" }}>
            Ship ≥{pct(settings.shipFloor, 1)} · Craft ≥{pct(settings.craftFloor, 0)}
          </p>
        </div>
